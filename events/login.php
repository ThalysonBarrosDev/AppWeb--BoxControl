<?php
    ...
    if (isset($_POST['InputEmail']) && !empty($_POST['InputEmail']) && isset($_POST['InputPassword']) && !empty($_POST['InputPassword'])) {
        ...
        if ($user->validateLogin($mail, $pass) == TRUE) {

            if (isset($_SESSION['idUserSession'])) {

                $flash_message = new FlashMessage();
                $flash_message->setMessage("Login realizado com sucesso!", 'success');
            ...
            $flash_message = new FlashMessage();
            $flash_message->setMessage("Dados de login incorretos!", 'error');
        ...
        $flash_message = new FlashMessage();
        $flash_message->setMessage("Erro ao realizar o login!", 'error');
...
<head>
    ...
    <title>Box Control - Validação</title>
...
<body style="padding:30px; background-color:#212529 !important;">
    ...
    <script><?php $flash_message->printMessage(); ?></script>
</body>
